...
`super-git preview stage-changes`는 첫 preview 명령이다. 현재 unstaged/untracked
변경을 stage하기 위한 plan을 만들지만, 실제로 stage하지 않는다.

```bash
cargo run -p super-git-cli -- preview stage-changes
```
...
cargo run -p super-git-cli -- execute --plan /tmp/super-git-plan.json > /tmp/super-git-token.json
cargo run -p super-git-cli -- undo --token /tmp/super-git-token.json
```
