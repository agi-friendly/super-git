...
`inspect`는 읽기 전용 계약이다. `next.allowed`는 실행 허가가 아니라 preview 후보이며,
쓰기 작업의 실제 위험도와 실행 잠금은 이후 preview/execute 단계에서 별도로 계산한다.
`next.needs_human_review`는 그 단계까지 예약된 bucket이므로 현재는 빈 배열로 유지한다.
