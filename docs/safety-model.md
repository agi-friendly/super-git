...
Any future execute support for worktree removal must require explicit human
confirmation and must revalidate the full target state immediately before
calling `git worktree remove` without `--force`.
