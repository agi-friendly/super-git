...
pub fn normalize_existing_path(path: &Path) -> Result<PathBuf> {
    ...
    if !path.is_dir() {
        return Err(SuperGitError::PathIsNotDirectory(path.to_path_buf()));
    }
...
mod tests {
    ...
    #[test]
    fn normalize_existing_path_rejects_files() {
        let temp_dir = tempfile::tempdir().expect("create temp dir");
        let file_path = temp_dir.path().join("README.md");
        fs::write(&file_path, "hello").expect("write file");

        let error = normalize_existing_path(&file_path).expect_err("reject file path");

        assert!(matches!(
            error,
            SuperGitError::PathIsNotDirectory(path) if path == file_path
        ));
    }
}
